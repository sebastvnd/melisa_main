...
mod operations_tests {
    ...
    fn test_create_node_success() {
        ...
        let result = mgr.create("web-1", 100_000, "http://localhost:3000", "example.com", "/api");
    ...
    fn test_create_node_hash_is_64_chars() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("hash-test", 100_001, "http://localhost:3001", "example.com", "/")
            .unwrap();
    ...
    fn test_create_node_empty_route_path_normalized_to_slash() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("route-empty", 100_002, "http://localhost:3002", "example.com", "")
            .unwrap();
    ...
    fn test_create_node_trailing_slash_normalized() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("trail", 100_003, "http://localhost:3003", "example.com", "/api/")
            .unwrap();
        assert_eq!(node.route_path, "/api");
    }
    ...
    fn test_create_node_domain_normalized_to_lowercase() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("case-test", 100_004, "http://localhost:3004", "EXAMPLE.COM", "/")
            .unwrap();
        assert_eq!(node.domain, "example.com");
    }
    ...
    fn test_create_node_url_trailing_slash_removed() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("url-slash", 100_005, "http://localhost:3005/", "example.com", "/")
            .unwrap();
    ...
    fn test_create_node_empty_name_rejected() {
        ...
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "Nama kosong harus InvalidInput");
    }
    ...
    fn test_create_node_pid_below_range_rejected() {
        let (mgr, _dir) = make_manager();
        let r = mgr.create("low-pid", PID_START - 1, "http://localhost:3008", "x.com", "/");
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "PID di bawah range harus ditolak");
    }
 
    #[test]
    fn test_create_node_pid_above_range_rejected() {
        let (mgr, _dir) = make_manager();
        let r = mgr.create("high-pid", PID_END + 1, "http://localhost:3009", "x.com", "/");
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "PID di atas range harus ditolak");
    }
    ...
    fn test_create_node_invalid_url_rejected() {
        ...
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "URL tidak valid harus ditolak");
    }
    ...
    fn test_create_node_ftp_url_rejected() {
        ...
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "Skema ftp:// harus ditolak");
    }
    ...
    fn test_create_node_domain_with_path_rejected() {
        ...
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "Domain dengan path harus ditolak");
    }
    ...
    fn test_create_node_route_without_leading_slash_rejected() {
        ...
        assert!(matches!(r, Err(NodeError::InvalidInput(_))), "Route tanpa '/' harus ditolak");
    }
    ...
    fn test_delete_existing_node_success() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("to-delete", 100_030, "http://localhost:3030", "x.com", "/del")
            .unwrap();
    ...
    fn test_delete_removes_node_from_list() {
        ...
        assert!(mgr.get(&node.hash).is_none(), "Node harus tidak ada setelah dihapus");
    }
    ...
    fn test_delete_nonexistent_node_returns_not_found() {
        ...
        assert!(matches!(r, Err(NodeError::NotFound)), "Delete hash tidak ada harus NotFound");
    }
    ...
    fn test_list_returns_all_hashes() {
        let (mgr, _dir) = make_manager();
        let n1 = mgr.create("list-a", 100_040, "http://localhost:3040", "x.com", "/a").unwrap();
        let n2 = mgr.create("list-b", 100_041, "http://localhost:3041", "x.com", "/b").unwrap();
    ...
    fn test_list_is_sorted() {
        let (mgr, _dir) = make_manager();
        mgr.create("list-z", 100_042, "http://localhost:3042", "x.com", "/").unwrap();
        mgr.create("list-a2", 100_043, "http://localhost:3043", "x.com", "/a2").unwrap();
        mgr.create("list-m", 100_044, "http://localhost:3044", "x.com", "/m").unwrap();
        let list = mgr.list().unwrap();
    ...
    fn test_list_after_delete_shrinks() {
        let (mgr, _dir) = make_manager();
        let n1 = mgr.create("shrink-a", 100_045, "http://localhost:3045", "x.com", "/").unwrap();
        mgr.create("shrink-b", 100_046, "http://localhost:3046", "y.com", "/").unwrap();
    ...
    fn test_get_existing_node() {
        let (mgr, _dir) = make_manager();
        let node = mgr
            .create("get-me", 100_050, "http://localhost:3050", "get.com", "/get")
            .unwrap();
    ...
    fn test_create_node_https_url_accepted() {
        let (mgr, _dir) = make_manager();
        let r = mgr.create("https-node", 100_060, "https://secure.example.com", "secure.com", "/");
